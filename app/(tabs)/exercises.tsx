...
export default function ExercisesScreen() {
  const { colors, findExercises, catalogCount } = useApp();
  const [query, setQuery] = useState('');
  const [muscle, setMuscle] = useState('all');
  const [items, setItems] = useState<Exercise[]>([]);
  useEffect(() => { const t = setTimeout(() => findExercises(query, muscle).then(setItems), 120); return () => clearTimeout(t); }, [query, muscle, findExercises, catalogCount]);
  return <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: colors.background }}>
    <FlatList data={items} keyExtractor={i => i.id} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} ListHeaderComponent={<>
      <Eyebrow>CATÁLOGO // {catalogCount} EXERCÍCIOS</Eyebrow><Text style={styles.title}>Encontre seu movimento</Text>
      ...
      <FlatList horizontal data={MUSCLES} keyExtractor={i => i} showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingVertical: 4, paddingBottom: 12 }} renderItem={({ item }) => <Chip label={item === 'all' ? 'Todos' : muscleLabel(item)} selected={muscle === item} onPress={() => setMuscle(item)} />} />
