...
export function ExerciseCard({ exercise, compact = false, onPress }: { exercise: Exercise; compact?: boolean; onPress?: () => void }) {
  ...
  const mediaLabel = hasVideo ? 'vídeo interno' : gallery.some(isAnimatedExerciseImage) ? 'animação disponível' : gallery.length ? `${gallery.length} imagem(ns)` : 'imagem local';
  ...
  return <Pressable onPress={onPress ?? (() => router.push({ pathname: '/exercise/[id]', params: { id: exercise.id } }))}><Card style={[styles.card, compact && { padding: 10 }]}>
    <ExerciseImage exercise={exercise} path={gallery[0]} style={[styles.image, compact && { width: 62, height: 62 }]} compact={compact} />
